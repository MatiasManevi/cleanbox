...
# CONF PHP-FPM
RUN sed -i "s/^listen\s*=.*$/listen = 127.0.0.1:9000/" /etc/php/7.2/fpm/pool.d/www.conf && \
	echo 'php_flag[display_errors] = on' >> /etc/php/7.2/fpm/pool.d/www.conf && \
	echo 'php_admin_value[error_log] = /var/log/fpm-php.www.log'  >> /etc/php/7.2/fpm/pool.d/www.conf && \
	echo 'php_admin_flag[log_errors] = on'   >> /etc/php/7.2/fpm/pool.d/www.conf

RUN sed -i "s/display_errors = .*/display_errors = stderr/" /etc/php/7.2/fpm/php.ini && \
    sed -i "s/memory_limit = .*/memory_limit = 2048M/" /etc/php/7.2/fpm/php.ini && \
    sed -i "s/;date.timezone.*/date.timezone = America\/Argentina\/Buenos_Aires/" /etc/php/7.2/fpm/php.ini && \
    ...
    sed -i "s/post_max_size = .*/post_max_size = 32M/" /etc/php/7.2/fpm/php.ini && \
...
# CONF PHP-CLI
RUN sed -i "s/;date.timezone.*/date.timezone = America\/Argentina\/Buenos_Aires/" /etc/php/7.2/cli/php.ini
