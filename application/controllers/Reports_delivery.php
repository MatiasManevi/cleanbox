            ...
            if($renter && strlen($renter['client_email']) > 0 && filter_var($renter['client_email'], FILTER_VALIDATE_EMAIL)){ 
                ...
                if($image){
                    $response['status'] = Mailing::send(array(
                        'subject' => "Recibo pago ".$credits[0]['cred_concepto']." | Inmobiliaria " . User::getBussinesName(),
                        'body' => 'Hola! '.$renter['client_name'].', recientemente usted pago su '.$credits[0]['cred_concepto'].', le adjuntamos el recibo en formato digital, gracias por ayudarnos a proteger el medio ambiente!',
                        'report_root' => $report_root,
                        'report_file_name' => $report_file_name,
                        'is_html' => false,
