        ...
        if (count($mants) > 0) {
            foreach ($mants->result_array() as $row) {
                ...
                $mant = array(
                    ...
                    'proveedor' => 'proveedores: ' . $row['mant_prov_1'] . ', ' . $row['mant_prov_2'] . ', ' . $row['mant_prov_3'],
//                    'proveedor_tel' => isset($proveedor1['prov_tel']) ? 'prov 1:'.$proveedor1['prov_tel'] : ''.' '.isset($proveedor2['prov_tel']) ? 'prov 2:'.$proveedor2['prov_tel'] : ' '.isset($proveedor3['prov_tel']) ? 'prov 3:'.$proveedor3['prov_tel'] : '',
...
//            var_dump($ordenados);die;
    ...
    function crear_caja() {
        ...
        if (empty($caja)) {
            $caja_comienza = $this->basic->get_where('caja_comienza', array('caj_dia' => date('d'), 'caj_mes' => date('m'), 'caj_ano' => date('Y')))->row_array();
            $mensual = array(
                'men_mes' => date('m'),
                'men_ano' => date('Y'),
                'men_creditos' => $caja_comienza['caj_saldo'],
                'men_debitos' => 0
            );
            $caja['men_date'] = Date('d-m-Y  h:i:s A');
            $caja['men_info'] = '';
            $this->basic->save('mensuales', 'men_id', $mensual);
        }
    }
    ...
    function caja_comienza() {
        ...
        if (empty($caja)) {
            $last_comienza = $this->basic->get_where('caja_comienza', array(), 'caj_id')->last_row('array');
            if (!empty($last_comienza)) {
                $fecha = $last_comienza['caj_dia'] . '-' . $last_comienza['caj_mes'] . '-' . $last_comienza['caj_ano'];

                $creditos = $this->basic->get_where('creditos', array('cred_fecha' => $fecha));
                $debitos = $this->basic->get_where('debitos', array('deb_fecha' => $fecha));
                ...
                foreach ($transferencias->result_array() as $row) {
                    $agregar = $this->comp_fecha($row['transf_fecha'], $fecha, $fecha);
                ...
                $monto_comienza = $last_comienza['caj_saldo'] + $entradasmenossalidas - $transferencias_a_caja + $transferencias_a_diaria;
                ...
                $saldo_banco_mes_pasado = 0;
                $desde = '01-' . $last_comienza['caj_mes'] . '-' . $last_comienza['caj_ano'];
                $hasta = '31-' . $last_comienza['caj_mes'] . '-' . $last_comienza['caj_ano'];
                if ($last_comienza['caj_mes'] < Date('m') || $last_comienza['caj_mes'] > Date('m') && $last_comienza['caj_ano'] < Date('Y')) {
                    // Si el last comienza es del mes pasado, no pasamos el saldo bancario acumulado del
                    // mes pasado a este nuevo mes. Solo la caja fisica
                    foreach ($creditos_bancarios->result_array() as $row) {
                        ...
                        if ($agregar == '11') {
                            if ($row['cred_concepto'] != 'Gestion de Cobro' && $row['cred_concepto'] != 'Gestion de Cobro Sobre Intereses') {
                                $saldo_banco_mes_pasado += $row['cred_monto'];
                            }
                        }
                    }
                    foreach ($debitos_bancarios->result_array() as $row) {
                        ...
                        if ($agregar == '11') {
                            if ($row['deb_concepto'] != 'Gestion de Cobro' && $row['deb_concepto'] != 'Gestion de Cobro Sobre Intereses') {
                                $saldo_banco_mes_pasado -= $row['deb_monto'];
                            }
                        }
                    }
                }
                if ($saldo_banco_mes_pasado > 0) {
                    $monto_comienza -= $saldo_banco_mes_pasado;
                }
            } else {
                $monto_comienza = 0;
            }
...
//        }
    ...
    function isBussy($prov_name) {
        ...
        foreach ($mantenimientos->result_array() as $row) {
            if ($row['mant_prov_1'] == $prov_name || $row['mant_prov_2'] == $prov_name || $row['mant_prov_3'] == $prov_name) {
                $bussy = true;
                break;
            }
        }
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $mantenimiento = array(
                    ...
                    'mant_prov_1' => strtoupper($this->input->post('mant_prov_1')),
                    'mant_prov_2' => strtoupper($this->input->post('mant_prov_2')),
                    'mant_prov_3' => strtoupper($this->input->post('mant_prov_3')),
...
//                    }
    ...
    /**
     * Recorre todos los contratos activos de cada propietario.
     * @param string $desde
     * @param string $hasta 
     */
    public function informar_pendientes($desde = false, $hasta = false) {
        if ($desde != false && $hasta != false) {
            $this->data['desde'] = $desde;
            $this->data['hasta'] = $hasta;
            $cuentas = $this->basic->get_all('cuentas_corrientes');
                ...
                if ($row['cc_prop'] != 'INMOBILIARIA' && $row['cc_prop'] != 'CAJA FUERTE') {
                    ...
                    foreach ($rendiciones->result_array() as $rend) {
                        if ($this->comp_fecha($rend['deb_fecha'], $desde, $hasta) == '11') {
                            ...
                            $pendiente = false;
                            foreach ($creditos->result_array() as $credito) {
                                if ($this->comp_fecha($credito['cred_fecha'], $rend['deb_fecha'], $hasta) == '11') {
                                    // vemos si de ahi en mas no le depositarion nuevamente
                                    if (in_array($credito['cred_concepto'], array('Alquiler', 'Alquiler Comercial', 'Indemnizacion', 'Intereses'))) {
...
// @TODO This should be fixed, now it will be sorted as string
    ...
    function get_last_payment($contrato, $concepto) {
        $creditos = $this->basic->get_where('creditos', array('cred_depositante' => $contrato['con_inq']), 'cred_id');
        $agr_cred = 0;
        $cred_filt = array();
        $tiene_creds = false;
        $last_payment = false;
        ...
        if ($tiene_creds) {
            ...
            $ult_ano = intval(trim($ano_last));
            $ult_id = $last_payment['cred_id'];
            $entro = false;
...
//            var_dump($last_payment);
    ...
    function calcular_monto($mes_debido, $periodos) {
        ...
        foreach ($periodos->result_array() as $row) {
            ...
            if (strlen($mes_debido) != 10) {
//                $track = debug_backtrace();
//                var_dump($track[1]['function']);
//                var_dump($periodos);
            }
    ...
    function calcular_intereses($deuda, $contrato, $mes_debido, $periodos, $fecha_informe) {
        ...
        if ($mes_debido_array[1] <= $fecha_informe_array[1] && $mes_debido_array[2] <= $fecha_informe_array[2] || $mes_debido_array[2] < $fecha_informe_array[2]) {
            $intereses = 0;
            $monto = 0;
