...
return $instance->load->view('reports/renter_payment_historial_report', $instance->data, TRUE);
    ...
    public static function buildAccountFlushReport($from, $to) {
        $instance = &get_instance();
        ...
        $instance->data['from'] = $from;
        $instance->data['to'] = $to;
        $instance->data['heading'] = 'Informe de balances entre las fechas ' . $from . ' y ' . $to;
        $instance->data['accounts'] = array();
        ...
        foreach ($accounts as $account) {
            ...
            if ($account['cc_prop'] != 'INMOBILIARIA' && $account['cc_prop'] != 'CAJA FUERTE') {
                ...
                $credits = $instance->basic->get_where('creditos', array('cc_id' => $account['cc_id'], 'is_transfer' => 0))->result_array();
                /* solo para davinia y rima */
                $credits2 = $instance->basic->get_where('creditos', array('cred_cc' => $account['cc_prop'], 'is_transfer' => 0))->result_array();
                foreach ($credits2 as $credit2) {
                    if (!in_array($credit2, $credits))
                        array_push($credits, $credit2);
                }
                ...
                $debits = $instance->basic->get_where('debitos', array('cc_id' => $account['cc_id'], 'is_transfer' => 0))->result_array();
                /* solo para davinia y rima */
                $debits2 = $instance->basic->get_where('debitos', array('deb_cc' => $account['cc_prop'], 'is_transfer' => 0))->result_array();
                ...
                foreach ($debits as $debit) {
                    if (General::isBetweenDates($debit['deb_fecha'], $from, $to)) {
                        if (strpos($debit['deb_concepto'], 'Prestamo') === FALSE) {
                            $account['outs'] += $debit['deb_monto'];
                        }
                    }
                }

                $account['balance'] = round($account['ins'] - $account['outs'], 2);  
                array_push($instance->data['accounts'], $account);              
            }
        }

        return $instance->load->view('reports/account_flush_report', $instance->data, TRUE);
    }
