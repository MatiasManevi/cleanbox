        ...
        <table class="table table-hover">
            ...
            if (count($mantenimientos) > 0) {
                for ($x = 0; $x < count($mantenimientos); $x++) {
                    if ($mantenimientos[$x]['mant_prioridad'] == 1) {
                        $prioridad = 'Alta';
                    } elseif ($mantenimientos[$x]['mant_prioridad'] == 2) {
                        $prioridad = 'Media';
                    } else {
                        $prioridad = 'Baja';
                    }
                    if ($mantenimientos[$x]['mant_status'] == 1) {
                        $status = 'Creada';
                    } elseif ($mantenimientos[$x]['mant_status'] == 2) {
                        $status = 'Asignada y en marcha';
                    } else {
                        $status = 'Terminada';
                    }
                    echo '<tr class="reg_' . $mantenimientos[$x]['mant_id'] . '">';
                    echo '<td>' . $mantenimientos[$x]['mant_domicilio'] . '</td>';
                    echo '<td>' . $mantenimientos[$x]['mant_prop'] . '</td>';
                    echo '<td>' . $mantenimientos[$x]['mant_inq'] . '</td>';
                    echo '<td>' . $mantenimientos[$x]['mant_prov_1'] . ' ' . $mantenimientos[$x]['mant_prov_2'] . ' ' . $mantenimientos[$x]['mant_prov_3'] . '</td>';
                    echo '<td>' . $prioridad . '</td>';
                    echo '<td>' . $status . '</td>';
                    echo '<td>';
                    echo '<a href="javascript:;" class="glyphicon glyphicon-edit" onclick="load_edit(\'' . site_url('manager/load_edit_mantenimientos/' . $mantenimientos[$x]['mant_id']) . '\')"></a> | ';
                    echo '<a href="javascript:;" class="glyphicon glyphicon-print" onclick="request_post(\'' . site_url('manager/reporte_mantenimiento/' . $mantenimientos[$x]['mant_id']) . '\')"></a> | ';
                    echo '<a href="javascript:;" class="glyphicon glyphicon-trash" onclick="del(\'' . $mantenimientos[$x]['mant_id'] . '\',\'' . site_url('manager/del_mantenimientos/' . $mantenimientos[$x]['mant_id']) . '\')"></a>  ';
                    echo '</tr>';
                }
            } else {
