...
<div id="printable" class="report_sheet" style="width: 900px">
        
    <h4>Informe de Honorarios al día: <?php echo date('d-m-Y') ?></h4>

    <?php foreach ($honorary_payments as $honorary_payment) { ?>
    <?php $contract = $honorary_payment['contract']; ?>
    <?php $honoraries = $honorary_payment['honoraries']; ?>
    <table class="table" style="margin-bottom: 40px;">
        <tr>
            <th colspan="4" class="cell">Contrato de: <?php echo $contract['con_tipo'] ?> - Propietario: <?php echo $contract['con_prop'] ?> - Inquilino: <?php echo $contract['con_inq']; ?></th>
        </tr>
        <tr>
            <?php if($contract['honorary_cuotes'] > 0) { ?>
            <th colspan="4" class="cell">Se establecio que se pague en <?php echo $contract['honorary_cuotes'] ?> cuotas</th>
            <?php } else { ?>
            <th colspan="4" class="cell">Se establecio que se pague en 1 cuota</th>
            <?php } ?>
        </tr>
        <?php if(!empty($honoraries)) { ?>
        <tr>    
            <th colspan="2" class="cell">Fecha</th>
            <th colspan="2" class="cell">Monto</th>
        </tr> 
        <?php foreach ($honoraries as $key => $honorary) { ?>
        <tr>
            <td colspan="2" class="cell"><?php echo $honorary['cred_fecha']; ?></td>
            <td colspan="2" class="cell">$ <?php echo $honorary['cred_monto']; ?></td>
        </tr>
        <?php } ?>
        <?php } else { ?>
        <tr>
            <td>No existen pagos de Honorarios por este contrato</td>
        </tr>
        <?php } ?>
    </table>   
    <?php } ?>
...
<div id="non-printable" class="report_actions">
    ...
    <a class="btn btn-primary button_report" href="<?php echo site_url('home') ?>">Volver</a>
</div>
